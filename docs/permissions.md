...
認証・認可に関する主なコードは以下の通りです（HTTP ステータス付き）。
...
| `token_revoked` | 401 | トークンが失効済み（パスワード/権限変更、ユーザー削除・再作成など） |
| `invalid_credentials` | 401 | ログインのユーザー名/パスワード不一致（存在有無は区別しない） |
| `requires_global_admin` | 403 | GlobalAdmin 権限が必要 |
| `not_self_or_admin` | 403 | 本人または GlobalAdmin のみ許可される操作 |
| `insufficient_privilege` | 403 | 対象データベースへの権限不足 |
...
## 4. 高速化のためのキャッシュ機構
Kasaneは各リクエストごとのデータベース参照負荷を軽減するため、サーバー内に高度に最適化された `AuthCache` 機構を持っています。
一度認証および権限チェックが行われたユーザーの情報は、内部で64分割されたシャーディング構造（Lock Striping）を持つインメモリキャッシュに保持されます。これにより、高並行アクセス時にもロックの競合が発生せず、極めて高速なAPIアクセスが可能です。
...
- **容量制限**: 最大10,000件のユーザー情報を保持します（各シャードごとに均等に制限）。上限に達した場合は、$O(1)$ のコストで**挿入順が最も古いエントリから順に自動で追い出されます**（全消去はしません）。

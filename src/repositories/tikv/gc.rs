...
/// 1 回の掃除で 1 テーブルに費やすチャンク数の上限。
///
/// 巨大なテーブルの回収で 1 周が延々と続かないようにする。残りは次の周回で消える。
const MAX_CHUNKS_PER_TABLE: usize = 64;
...
impl TikvDb {
    ...
    pub async fn sweep_retired_tables(&self) -> Result<usize, AppError> {
        ...
        for table_id in retired {
            for chunk in 0..MAX_CHUNKS_PER_TABLE {
                let removed = self
                ...
                if chunk + 1 == MAX_CHUNKS_PER_TABLE {
                    // 残りは次の周回に任せる。
    ...
    pub fn spawn_sweeper(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            loop {
                ...
                match db.sweep_retired_tables().await {
                    Ok(0) => {}

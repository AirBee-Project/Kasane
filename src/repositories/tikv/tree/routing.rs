...
use super::{AppError, Reader, Readers, ShardEntry, ShardReader, ShardValue, TableId};
...
///
/// `reader` は読み取り専用呼び出し(`data_get`)だけが渡す。書き込み経路(`tree::write`)は
/// 常に `None` を渡し、キャッシュを一切経由せず生の `load_nodes` を呼ぶ。
pub(super) async fn route_leaves_batched<R: Reader>(
    txn: &Readers<R>,
    table_id: TableId,
    ids: &[FlexId],
    reader: Option<&ShardReader<'_>>,
...
async fn descend_batched<R: Reader>(
    ...
    while !level.is_empty() {
        ...
        let mut nodes = match reader {
            Some(reader) => reader.nodes(txn, table_id, &regions).await?,
            None => load_nodes(txn, table_id, &regions).await?,
        };
...
///
/// `reader` は呼び出し元（[`TikvTableSource`](super::super::TikvTableSource)）が同一クエリの
/// 間だけ持ち回る [`ShardReader`]。`Source::read_range_ids` は 1 回のクエリで複数回呼ばれ、

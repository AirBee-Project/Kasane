...
impl TikvWrite<'_> {
    ...
    ///
    /// 触るのは「対象の空間 ID が属するリーフ」だけで、テーブル全体は排他しない
    /// （[`kv::lock_shards`]）。これが安全なのは、この木が次の不変条件を満たすため：
    ///
    /// > **リーフ R の内容または到達可能性を変える操作は、必ず R 自身を書くか消す。**
    ///
    /// - 分割できるのはリーフだけなので、R が分割されるなら R が書き換わる
    /// - 統合（[`try_merge_up`](Self::try_merge_up)）は親へ畳むときに子キーを削除する ＝ R を消す
    /// - 祖先方向の統合は「子のいずれかがポインタノードなら行わない」ので、R より上を畳むには
    ///   先に R の親を畳む必要があり、それは R を消すことを意味する
    ///
    /// よって R をロックすれば R は誰にも変えられない。降下（祖先のポインタノード読み）は
    /// ロックなしのスナップショット読みでよく、古い形で誤ったリーフを選んでもロック後の検証で
    /// 検出できる。検出したら [`mark_stale`](TikvWrite::mark_stale) で試行ごと捨てる。
    ...
    /// **降下・ロック・リーフの書き直しをそれぞれ 1 回で済ませる。**
    ///
    /// 要素ごとに `data_insert_impl` を呼ぶと降下とロック取得が要素数だけ直列に積み上がり、
    /// 同じリーフを何度も復元・直列化し直すことになる。
    ...
    ///
    /// 統合は親と**その全子**を書き換えるので、判断の前にその集合をまとめてロックする。空の
    /// 子領域も含めるのは（キーが無くてもロックは取れる）、統合で消える予定の空領域へ他者が
    /// 書き込めてしまうため。
    ///
    /// 親子関係は降下で判ったものを使い、ルートから引き直さない（同じ `start_ts` を読む以上
    /// 答えは変わらない）。
    ...
    ///
    /// テーブル単位の排他を取らないので**ある一点での検証**にしかならない。新しい制約はコミット
    /// 後の書き込みには適用されるので、すり抜けうるのは「古い制約を読んだうえで検証後に
    /// コミットされた書き込み」だけ。
    #[tracing::instrument(skip_all, fields(table_id = %table_id))]

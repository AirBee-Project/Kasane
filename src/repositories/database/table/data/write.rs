...
impl KasaneDbWrite {
        ...
        loop {
            ...
            let can_merge = siblings.iter().all(|sibling| {
                ...
                match redb_sppatialid_to_value
                    .get((table_id.into_bytes(), sibling.spatial_encode()))
                {
    ...
    pub(self) fn remove<'a>(
        ...
        if let Some(access_guard) =
            redb_sppatialid_to_value.remove((table_id.into_bytes(), target.spatial_encode()))?
        {
    ...
    pub(self) fn overlap_equal<'a>(
        ...
        if let Some(access_guard) =
            redb_sppatialid_to_value.get((table_id.into_bytes(), target.spatial_encode()))?
        {
    ...
    pub(self) fn overlap_parent<'a>(
        ...
        for parent in target.spatial_parents() {
            if let Some(access_guard) =
                redb_sppatialid_to_value.get((table_id.into_bytes(), parent.spatial_encode()))?
            {

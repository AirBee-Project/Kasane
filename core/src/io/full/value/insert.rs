...
impl Storage {
    ...
    fn process_range(range: Range) -> Result<EncodeIDSet, UserError> {
        match range {
            Range::Function(f) => process_function(f),
            Range::Calculation(c) => process_calculation(c),
            Range::Ids(ids) => {
    ...
    match function {
        Function::Point(point) => {
            ...
            set.insert(function::point::point(
                point.z,
                check_coordinate(point.point1)?,
            ));
            Ok(set)
        }
        Function::Line(line) => Ok(kasane_logic::function::line::line(
            line.z,
            check_coordinate(line.point1)?,
            check_coordinate(line.point2)?,
        )),
        Function::Triangle(triangle) => Ok(kasane_logic::function::triangle::triangle(
            triangle.z,
            check_coordinate(triangle.point1)?,
            check_coordinate(triangle.point2)?,
            check_coordinate(triangle.point3)?,
        )),
    }
}

...
/// `PUT /layers/{layer_name}/data` でデータを挿入し、200 OKを検証してレスポンスを返す。
///
/// # Arguments
...
/// });
///
/// let result_json = search_data(&test_app, "my_layer", &query).await;
...
pub async fn search_data(
    ...
    let req = Request::builder()
        .method("POST")
        .uri(format!("/layers/{}/data/search", layer_name))
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from(serde_json::to_string(&body).unwrap()))
        .unwrap();
